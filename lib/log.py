"""
Lightweight colorized console output.

Plain ANSI escape codes (not `rich` markup) are used on purpose: several
callers stream raw, unpredictable text from subprocesses or web pages
(APK filenames, GitHub error bodies, etc.), and rich's `[tag]` markup
syntax would misinterpret literal square brackets in that text. Raw ANSI
codes just wrap the text and can't be broken by its contents.

GitHub Actions' log viewer (web and mobile) renders ANSI colors natively,
so this works there without any extra configuration.
"""

import os
import re

# Respect NO_COLOR / non-TTY environments (e.g. output piped to a file).
...
def header(msg: str) -> None:
    """Section banner, e.g. starting a new app."""
    print(_wrap(f"\n▶ {msg}", _C.BOLD, _C.CYAN))


def step(msg: str) -> None:
    """A notable action being taken (fetching, trying a URL, etc.)."""
...
# --- Line-by-line colorizer for the morphe-desktop patcher's own output ---
# patcher.py streams the Java CLI's stdout verbatim; this inspects each
# line and re-prints it with a matching icon/color instead of adding our
# own lines around it, so the live streaming behavior is unchanged.
